...
task_customized_model = kpu.load("/sd/preset/models/traffic/trafficsign_01.kmodel")
...
classes_customized_model = ["Park", "Stop", "Prohibit", "Straight", "", "", "Unload"]
...
while True:
    ...
    if (_timer_current_time_elapsed) - old_time >= 80:
        ...
        if code_customized_model:
            for i in (code_customized_model):
                ...
                camera.draw_rectangle((i.x()),(i.y()), (i.w()), 20, color=(51,102,255), thickness=2, fill=True)
                lcd_draw_string(camera,((i.x()) + 2),((i.y()) + 2), (classes_customized_model[i.classid()]), color=(255,255,255), scale=1, mono_space=False)
                name = classes_customized_model[i.classid()]
