...
def draw_ok(type):
	if type == "released":
		...
		splash.draw_string(MENU_ITEM_COORD[6][0]+8,MENU_ITEM_COORD[6][1]+6,"RUN", mono_space=False,color=color_splash_theme,scale=1.6)
	elif type == "pressed":
...
def draw_title_bar(index):
	...
	splash.draw_string(MENU_ITEMS[index][1][4],MENU_ITEMS[index][1][5],MENU_ITEMS[index][1][6],color=color_white,mono_space=False,scale=1.6)
...
def draw_item(row, type, realindex):
	if type == "unselected":
		...
		splash.draw_string(MENU_ITEM_COORD[row][2], MENU_ITEM_COORD[row][3], MENU_ITEMS[realindex][0], color=color_gray, scale=1.6, mono_space=False)
	elif type == "selected":
		splash.draw_rectangle(MENU_ITEM_COORD[row][0], MENU_ITEM_COORD[row][1], MENU_ITEM_BOX_SIZE[0],MENU_ITEM_BOX_SIZE[1], color=color_splash_theme, fill=True, thickness=1)
		...
		splash.draw_string(MENU_ITEM_COORD[row][2], MENU_ITEM_COORD[row][3], MENU_ITEMS[realindex][0], color=color_white, scale=1.6, mono_space=False)
	elif type == "clearup":
		row = row - 1 
		splash.draw_rectangle(MENU_ITEM_COORD[row][0], MENU_ITEM_COORD[row][1], MENU_ITEM_BOX_SIZE[0],MENU_ITEM_BOX_SIZE[1], color=color_splash_theme, fill=True, thickness=1)
		splash.draw_rectangle(MENU_ITEM_COORD[row][0], MENU_ITEM_COORD[row][1], MENU_ITEM_BOX_SIZE[0],MENU_ITEM_BOX_SIZE[1], color=color_gray, fill=False, thickness=1)
...
def draw_run(row, type, text):
	if type == "running":
		...
		splash.draw_string(MENU_ITEM_COORD[row][2], MENU_ITEM_COORD[row][3], text, color=color_white, scale=1.6, mono_space=False)
	elif type == "failed":
...
while True:
	...
	if (key_state_down == 1 and ksd == 0):
		try:
			...
			print("Running command: exec(open(\"" + str(MENU_ITEMS[MENU_CURRENT_SELECT][2]) + "\").read())")
			file_to_be_opened = MENU_ITEMS[MENU_CURRENT_SELECT][2]
			print(gc.mem_free())
			time.sleep(1)
			exec(open(file_to_be_opened).read())
